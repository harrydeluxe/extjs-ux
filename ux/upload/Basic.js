...
Ext.define('Ext.ux.upload.Basic', {	
	...
	constructor: function(owner, config)
    {
		...
		me.actions = {
				textUploadFile: Ext.create('Ext.Action', {
	...
	{
		var me = this, file = me.uploader.getFile(id);
	...
	initializeUploader: function()
	{
		...
		Ext.each([ 'Init', 'ChunkUploaded', 'FilesAdded', 'FilesRemoved', 'FileUploaded', 'PostInit', 'QueueChanged', 'Refresh',
				'StateChanged', 'BeforeUpload', 'UploadFile', 'UploadProgress', 'Error' ], function(v)
		{
	...
	_FileUploaded: function(uploader, file, status)
	{
		...
		else
		{
			...
			this.fireEvent('uploaderror', this, Ext.apply(status, {file: file}));
		}
	...
	{
		this.actions.textUploadFile.setText(file.name);
	...
	_Error: function(uploader, data)
	{
		if(data.file)
		{
			...
			{
				data.file.msg = Ext.String
						.format('<span style="color: red">{2} ({0}: {1})</span>', data.code, data.details, data.message);
			}
